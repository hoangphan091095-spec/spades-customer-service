// free_round_history_screen.dart
import 'package:flutter/material.dart';
import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:provider/provider.dart';
import 'auth_service.dart';

class FreeRoundHistoryScreen extends StatefulWidget {
  @override
  _FreeRoundHistoryScreenState createState() => _FreeRoundHistoryScreenState();
}

class _FreeRoundHistoryScreenState extends State<FreeRoundHistoryScreen> {
  final int _pageSize = 20;
  int _currentLimit = 20;
  bool _isLoadingMore = false;
  bool _hasMoreData = true;
  final ScrollController _scrollController = ScrollController();
  List<Map<String, dynamic>> _allRoundHistory = [];
  ...
  Future<void> _loadInitialData() async {
    ...
    try {
      ...
      if (docSnapshot.exists) {
        ...
        final roundHistory = List<Map<String, dynamic>>.from(data?['freeRoundHistory'] ?? []);

        // Sort by date descending (newest first) and take max 30
        roundHistory.sort((a, b) => _compareDates(b['date'], a['date']));
        final limitedHistory = roundHistory.take(30).toList();

        setState(() {
          _allRoundHistory = limitedHistory;
          _isInitialLoading = false;
          _hasMoreData = _currentLimit < _allRoundHistory.length;
        });
      } else {
        setState(() {
          _isInitialLoading = false;
  ...
  Widget build(BuildContext context) {
    final displayedHistory = _allRoundHistory.take(_currentLimit).toList();
